...
class _ScannerPageState extends State<ScannerPage>
  ...
  Future<void> _processImage(File imageFile) async {
    ...
    try {
      ...
      final savedFile = await processedFile.copy(
        '${prescriptionDir.path}/$fileName',
      );
      ...
      try {
        ...
        if (rawText.isEmpty) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('No text found in the scanned area'),
              ),
            );
          }
          return;
        ...
        final uri = Uri.parse('http://192.168.1.8:3000/emr');
        ...
        if (response.statusCode == 201) {
          ...
          final timestamp =
              emr['timestamp']?.toString() ?? DateTime.now().toIso8601String();
          ...
          final newRecord = {
            ...
            'diagnosis':
                emr['diagnosis']?.toString() ?? 'No diagnosis provided',
            'prescriptions': (emr['prescriptions'] as List<dynamic>?)
                    ?.map((e) => e.toString())
                    .toList() ??
                [],
          };
  ...
  Future<File?> _enhanceAndCropImage(File originalImage) async {
    try {
      ...
      final safeWidth =
          x + width > capturedImage.width ? capturedImage.width - x : width;
      final safeHeight =
          y + height > capturedImage.height ? capturedImage.height - y : height;
                    ...
                    onTap: () {
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (_) => EMRForm(
                            patientId:
                                record['patientId']?.toString() ?? 'unknown',
